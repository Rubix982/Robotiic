...
class MCV:
    '''
    Most Constrained Variables heuristic
    '''
        ...
        while True:
            ...
            for i in range(0, len(self.Nodes)):
                ...
                if maxx > self.Nodes[i].edges and self.Nodes[i].color == '':
                    ...
                    # select it
                    maxx = self.Nodes[i].edges
                    maxx_node_index = i

            # We check if a potential node was found
            if maxx_node_index == -1:
            ...
            __local_neighbor_store = self.Nodes[maxx_node_index].neighbors

            # Step 2.1, traversing the neigboring adjacent nodes
            for i in range(0, len(self.Nodes)):

                if i != maxx_node_index and self.Nodes[i].name in __local_neighbor_store and self.Nodes[i].color in __local_color_store:
            ...
            # 3. Out of the colors that remain, randomly select one
            # and assign it to the current selected node
            self.Nodes[maxx_node_index].color = random.choice(__local_color_store)

            # 4. Update the edge count in the neighboring adjacent nodes
            for i in range(0, len(self.Nodes)):

                if i != maxx_node_index and self.Nodes[i].name in __local_neighbor_store:
